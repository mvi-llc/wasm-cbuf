...
protected:
  ...
  bool FastConversionInternal(const ast_struct* src_st, CBufParser& dst_parser,
...
#if defined(HJSON_PRESENT)
  bool FillHjsonInternal(const ast_struct* st, Hjson::Value& val, const char* ename,
                         bool dofill = true);
#endif
...
public:
  ...
  // st_name: optional name of the struct to print the header on, by default the main one on the
  // parser ename: optional member or sub_member of the struct
  ...
  unsigned int FillJstr(const char* st_name, unsigned char* buffer, size_t buf_size,
                        std::string& jstr);
  // Read a cbuf by name st_name (should have been parsed) from input stream buffer and write it to
  // cbuf parsed on dst_parser with name dst_name, on the bytes represented by dst_buf and dst_size
  unsigned int FastConversion(const char* st_name, unsigned char* buffer, size_t buf_size,
                              CBufParser& dst_parser, const char* dst_name, unsigned char* dst_buf,
                              size_t dst_size);

#if defined(HJSON_PRESENT)
  unsigned int FillHjson(const char* st_name, unsigned char* buffer, size_t buf_size,
                         Hjson::Value& val, bool print_body, const char* ename = nullptr);
#endif
};
