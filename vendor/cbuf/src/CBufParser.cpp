...
template <class T>
bool process_element_json(const ast_element* elem, u8*& bin_buffer, size_t& buf_size,
...
bool process_element_string_json(const ast_element* elem, u8*& bin_buffer, size_t& buf_size,
                                 Hjson::Value& jval, bool dofill) {
  if (elem->array_suffix) {
    Hjson::Value arr(Hjson::Value::VECTOR);
    u32 array_size;
...
bool process_element_short_string_json(const ast_element* elem, u8*& bin_buffer, size_t& buf_size,
                                       Hjson::Value& jval, bool dofill) {
  if (elem->array_suffix) {
    Hjson::Value arr(Hjson::Value::VECTOR);
    u32 array_size;
...
bool CBufParser::FillHjsonInternal(const ast_struct* st, Hjson::Value& val, const char* ename,
                                   bool do_fill) {
  // All structs have a preamble, skip it
  if (!st->naked) {
    u32 sizeof_preamble = sizeof(cbuf_preamble);  // 8 bytes hash, 4 bytes size
...
bool CBufParser::PrintCSVHeader(const char* st_name, const char* ename) {
  ...
  if (st == nullptr) {
    WriteError("Could not find struct %s on the symbol table\n", main_struct_name.c_str());
    return false;
  }
...
bool CBufParser::FastConversionInternal(const ast_struct* src_st, CBufParser& dst_parser,
                                        const ast_struct* dst_st, unsigned char* dst_buf,
                                        size_t dst_size) {
  // All structs have a preamble, skip it
  if (!src_st->naked) {
    u32 sizeof_preamble = sizeof(cbuf_preamble);  // 8 bytes hash, 4 bytes size
    if (!dst_st->naked) {
      *(cbuf_preamble*)dst_buf = *(cbuf_preamble*)buffer;
    }
    buffer += sizeof_preamble;
    buf_size -= sizeof_preamble;
  }
    ...
    switch (elem->type) {
      ...
      case TYPE_CUSTOM: {
        ...
        if (dst_elem_st == nullptr) {
          // We have a dst elem, but it is not a struct. Struct to non struct is not a supported
          // conversion We could just skip it, for now we fail
          return false;
        }
        ...
        for (u32 i = 0; i < array_size; i++) {
          if (check_dst_array && i >= dst_array_size) {
            ...
            success =
              FastConversionInternal(inst, dst_parser, dst_elem_st, elem_dst_buf, elem_dst_size);
...
unsigned int CBufParser::FastConversion(const char* st_name, unsigned char* buffer, size_t buf_size,
                                        CBufParser& dst_parser, const char* dst_name,
  ...
  computeSizes(dst_st, dst_parser.sym, nullptr);
