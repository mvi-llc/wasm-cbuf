...
static bool isValTypeOperable(ValueType vt) {
  return (vt == VALTYPE_INTEGER) || (vt == VALTYPE_FLOAT);
}
      ...
      if (lex->checkToken(TK_NUMBER)) {
        ...
        while (!lex->checkToken(TK_CLOSE_SQBRACKET)) {
          if (lex->checkToken(TK_STAR)) {
            ...
            Error(
              "Only a close bracket or a multiplication operation are supported for now, found "
              "%s\n",
              TokenTypeToStr(lex->getTokenType()));
            return nullptr;
          }
